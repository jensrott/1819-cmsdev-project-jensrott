...
export class ProvideSleepResultsComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<ProvideSleepResultsComponent>,
  ...
  addSleepResults() {
    // Two-step approach needed, two times subscribe
    // Adds the sleep results in the database
    this.provideSleepResultsService.createSleepResultsPostType().subscribe(
      data => {
        // Create new Sleep-result in PostType
        this.sleepresults1 = data;
        this.provideSleepResultsService
          .createSleepResultsAcf(
            this.sleepresults1.id, // Write the ACF fields in the Sleep-goal PostType
            this.sleepresults.day,
            this.sleepresults.sleep_time,
            this.sleepresults.wake_up_time
          )
          .subscribe(
            data1 => {
